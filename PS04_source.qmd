...
-   The relationship in @fig-TSG appears to be positive and reasonably linear.
:::
...
-   No, it is not reasonable to conclude that social policies that increase white poverty will cause an increase in Trump support because a correlation does not equal a causation. There can be other variables affecting this result that we can't see.
:::
...
sixWhitePov <- predict(poverty_mod, newdata = data.frame(poverty = 6))
sixWhitePov
```

-   The predicted percent of Trump support at 6% white poverty is `r round(sixWhitePov, 2)`.
:::
...
-   No, it isn't a good idea to predict a Trump support at 85% white poverty because we will get a predicted percent of Trump support of way above 100% based on looking at @fig-ts.
:::
...
-   I would expect a negative correlation coefficient (for `non_white` and `trump_support`).
...
nw_mod_regression <- get_regression_table(nw_mod)
nw_mod_regression

```

-   The intercept is `r round((nw_mod_regression$estimate[1]), 2)`% (When the non white level is 0, the predicted average Trump support is `r round((nw_mod_regression$estimate[1]), 2)`%).

-   The slope is `r round((nw_mod_regression$estimate[2]), 2)`. (For every increase in non white level of 1 percentage point, there is an associated decrease in Trump support of `r round((nw_mod_regression$estimate[2]), 2)` percentage points).
:::
...
-   The predicted percent of Trump support for a region that has 70% of humans that identify as a person of color is `r round(nw_70, 2)`.
:::
...
-   Based on the evidence so far, the non_white explanatory variable seems to be a better explanatory variable of Trump support because there are less outside variables that could effect the correlation like seen in the poverty example. There is also a negative slope with the non_white explanatory variable which makes more sense than the rapidly increasing slope seen in the poverty example.
:::
...
```{r}
# Type your code and comments inside the code chunk
nw_61 <- predict(nw_mod, newdata = data.frame(non_white = 61))
nw_61

```

**A.** Her prediction of Trump support be for a region in which 61% of the people identify as non-white would be `r round(nw_61, 2)`.

```{r}
# Type your code and comments inside the code chunk
observed_value <- 58.3 + (-0.269 * 61)
observed_value

residual <- observed_value - nw_61
residual

```

**B.** Her prediction would be an underestimate because the residual is a negative number (specifically `r round(residual, 2)`).

**C.** The residual for this prediction is `r round(residual, 2)`.
:::
